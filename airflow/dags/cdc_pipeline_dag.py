...
DEBEZIUM_CONNECT_URL = os.getenv("DEBEZIUM_CONNECT_URL")  # http://debezium.cdc.local:8083

# Database (from aws_secretsmanager_secret)
DB_SECRET_NAME = os.getenv("DB_SECRET_NAME", f"{PROJECT_NAME}-{ENVIRONMENT}-db-credentials")

# Glue Job Names (from module.glue)
GLUE_JOB_BRONZE = os.getenv("GLUE_JOB_BRONZE", f"{PROJECT_NAME}-{ENVIRONMENT}-bronze-to-silver")
GLUE_JOB_GOLD = os.getenv("GLUE_JOB_GOLD", f"{PROJECT_NAME}-{ENVIRONMENT}-silver-to-gold")
...
def check_kafka_health(**context):
    ...
    try:
        ...
        for broker in brokers:
            ...
            try:
                ...
                if result == 0:
                    ...
                    logger.warning(f"✗ Broker {broker} connection failed (code: {result})")
            except Exception as e:
        ...
        logger.info(f"Kafka health check passed: {len(healthy_brokers)}/{len(brokers)} brokers")
...
def check_debezium_health(**context):
    ...
    try:
        ...
        for conn_name in connectors:
            try:
                r = session.get(
                    f"{DEBEZIUM_CONNECT_URL}/connectors/{conn_name}/status", 
                    timeout=10
                )
                r.raise_for_status()
                status = r.json()
        ...
        if failed_connectors:
            raise AirflowFailException(f"Failed/unhealthy connectors: {failed_connectors}")
...
def setup_debezium_connector(**context):
    ...
    try:
        ...
        r = session.get(f"{DEBEZIUM_CONNECT_URL}/connectors/{connector_name}", timeout=10)
        ...
        if r.status_code == 200:
            ...
            r = session.get(f"{DEBEZIUM_CONNECT_URL}/connectors/{connector_name}/status", timeout=10)
            status = r.json()
...
def monitor_data_quality(**context):
    ...
    try:
        ...
        connector_statuses = ti.xcom_pull(
            task_ids="debezium_health_check", 
        ...
        if not metrics["connectors_healthy"]:
            raise AirflowFailException("Data quality check failed: connectors unhealthy")
...
bronze_to_silver = GlueJobOperator(
    ...
    s3_bucket=S3_BUCKET,
    iam_role_name=GLUE_ROLE_ARN.split('/')[-1] if GLUE_ROLE_ARN else None,  # Extract role name from ARN
...
# Failure handling
[kafka_health, debezium_health, setup_connector, 
 bronze_to_silver, wait_bronze, quality_check,
 silver_to_gold, wait_gold] >> failure_alert >> cleanup_task