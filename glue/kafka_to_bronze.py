...
class KafkaToBronze:
    ...
    def parse_events(self, kafka_df: DataFrame) -> DataFrame:
        ...
        return parsed.select(
            ...
            # Deduplication key
            concat_ws("-", col("topic"), col("kafka_partition"), col("kafka_offset"))
                .alias("event_id"),
    ...
    def validate_events(self, df: DataFrame) -> tuple:
        ...
        bad_df = (df.filter(bad_condition)
            .withColumn("error_type",
                ...
                .when((col("after_json").isNull()) & (col("before_json").isNull()), "EMPTY_PAYLOAD")
                .otherwise("UNKNOWN"))
    ...
    def write_dlq(self, bad_df: DataFrame):
        ...
        return (bad_df.select(
                "topic", "kafka_partition", "kafka_offset", "raw_value",
                "error_type", "processed_at", "dlq_timestamp"
            ).writeStream
            .format("json")
