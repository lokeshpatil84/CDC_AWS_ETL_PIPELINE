...
class SilverToGold:
    ...
    def _init_glue(self):
        ...
        spark.conf.set("spark.sql.catalog.glue.catalog-impl", 
                      "org.apache.iceberg.aws.glue.GlueCatalog")
        spark.conf.set("spark.sql.catalog.glue.warehouse", f"s3://{self.bucket}/iceberg/")
    ...
    def build_user_analytics(self):
        ...
        if not self.table_exists("silver_users") or not self.table_exists("silver_orders"):
            logger.warning("Missing dependencies for user analytics")
            return
            
        # Read current records only
        users = (self.spark.read.table(f"glue.{self.database}.silver_users")
            .filter("_is_current = true"))
        orders = (self.spark.read.table(f"glue.{self.database}.silver_orders")
            .filter("_is_current = true"))
        
        # Calculate user order statistics
        user_stats = (orders.groupBy("user_id")
            .agg(
        ...
        # Build analytics
        result = (users.join(user_stats, users["id"] == user_stats["user_id"], "left")
            .select(
                ...
                datediff(current_date(), user_stats["last_order_date"]).alias("recency_days"),
    ...
    def build_product_analytics(self):
        ...
        if not self.table_exists("silver_products") or not self.table_exists("silver_orders"):
            logger.warning("Missing dependencies for product analytics")
            return
            
        products = (self.spark.read.table(f"glue.{self.database}.silver_products")
            .filter("_is_current = true"))
        orders = (self.spark.read.table(f"glue.{self.database}.silver_orders")
            .filter("_is_current = true"))
        
        # Overall product stats
        product_stats = (orders.groupBy("product_id")
            .agg(
                count("*").alias("total_orders"),
        ...
        # Last 30 days stats
        recent_stats = (orders.filter(datediff(current_date(), col("_valid_from")) <= 30)
            .groupBy("product_id")
            .agg(sum("total_amount").alias("revenue_last_30d")))
        
        result = (products
            .join(product_stats, products["id"] == product_stats["product_id"], "left")
            ...
            .select(
                ...
                coalesce(recent_stats["revenue_last_30d"], lit(0.0)).alias("revenue_last_30d"),
                # Performance tier
                when(coalesce(product_stats["total_revenue"], lit(0.0)) > 10000, "Star")
                    .when(coalesce(product_stats["total_revenue"], lit(0.0)) > 5000, "Good")
                    .when(coalesce(product_stats["total_revenue"], lit(0.0)) > 1000, "Average")
                    .otherwise("Low").alias("performance_tier"),
                current_timestamp().alias("refresh_date")
            ))
    ...
    def build_sales_summary(self):
        ...
        orders = orders.withColumn("order_date", date_format(col("_valid_from"), "yyyy-MM-dd"))
        ...
        # Top product per day
        product_daily = (orders.groupBy("order_date", "product_id")
            .agg(sum("total_amount").alias("product_revenue")))
        
        top_product = (product_daily
            .withColumn("rank", row_number().over(
                Window.partitionBy("order_date").orderBy(col("product_revenue").desc())
            ))
            .filter("rank = 1")
            .select("order_date", col("product_id").alias("top_product_id")))
        
        result = (daily.join(top_product, "order_date", "left")
            .select(
    ...
    def run(self):
        ...
        # Final stats
        logger.info("-" * 70)
        for table in ["gold_user_analytics", "gold_product_analytics", "gold_sales_summary"]:
            try:
                count = self.spark.sql(f"SELECT COUNT(*) FROM glue.{self.database}.{table}").collect()[0][0]
