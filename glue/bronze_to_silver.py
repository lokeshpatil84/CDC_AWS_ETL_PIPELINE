...
class BronzeToSilver:
    ...
    TABLE_SCHEMAS = {
        ...
        "products": {
            ...
            "types": {"id": "long", "price": "double", "created_at": "long", "updated_at": "long"},
            "transforms": {
        ...
        "orders": {
            "columns": ["id", "user_id", "product_id", "quantity", "total_amount", "status", "created_at", "updated_at"],
            "types": {"id": "long", "user_id": "long", "product_id": "long", 
                     "quantity": "int", "total_amount": "double", "created_at": "long", "updated_at": "long"},
            "transforms": {
        ...
        if self.table_exists(f"silver_{table}"):
            try:
                ...
                if watermark_row and watermark_row[0][0]:
                    ...
                    logger.info(f"Incremental load: source_ts_ms > {lookback} (watermark: {watermark})")
    ...
    def parse_and_transform(self, df, table: str):
        ...
        data_col = when(col("operation") == "d", col("before_json")).otherwise(col("after_json"))
        
        # Get sample for schema inference (handle nulls)
        sample_rows = df.filter(data_col.isNotNull()).select(data_col).limit(10).collect()
        if not sample_rows:
        ...
        window = Window.partitionBy("id").orderBy(
            col("pg_lsn").desc(),
            col("source_ts_ms").desc(),
            col("kafka_offset").desc()
        )
        
        deduped = (result.withColumn("row_num", row_number().over(window))
            .filter("row_num = 1")
            .drop("row_num"))
    ...
    def generate_hash(self, df, table: str):
        ...
        config = self.TABLE_SCHEMAS[table]
        business_cols = config["columns"] + list(config["transforms"].keys()) + ["is_active"]
        
        # Build concatenation with null handling
        concat_exprs = [coalesce(col(c).cast("string"), lit("NULL")) for c in business_cols]
    ...
    def merge_scd2(self, df, table: str):
        ...
        config = self.TABLE_SCHEMAS[table]
        base_cols = (config["columns"] + list(config["transforms"].keys()) + 
                    ["is_active", "operation", "source_ts_ms", "processed_at",
                     "kafka_partition", "kafka_offset", "pg_lsn", "_hash"])
    ...
    def process_table(self, table: str):
        ...
        # Log stats
        try:
            total = self.spark.sql(f"SELECT COUNT(*) FROM glue.{self.database}.silver_{table}").collect()[0][0]
            current = self.spark.sql(f"SELECT COUNT(*) FROM glue.{self.database}.silver_{table} WHERE _is_current = true").collect()[0][0]
